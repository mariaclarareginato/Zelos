...
// Deletar chamado e histórico
router.delete("/:id", (req, res) => {
  const chamadoId = req.params.id;

  const deleteHistoricoQuery = "DELETE FROM historico_chamados WHERE chamado_id = ?";
  db.query(deleteHistoricoQuery, [chamadoId], (err) => {
    if (err) return res.status(500).json({ error: "Erro ao deletar histórico do chamado" });

    const deleteChamadoQuery = "DELETE FROM chamados WHERE id = ?";
    db.query(deleteChamadoQuery, [chamadoId], (err2, result) => {
      if (err2) return res.status(500).json({ error: "Erro ao deletar chamado" });
...
// Listar todos chamados pendentes (disponíveis)
router.get("/disponiveis", (req, res) => {
  const query = `
    SELECT c.id, c.titulo, c.status, u.nome AS usuario
    FROM chamados c
  ...
  `;
  db.query(query, (err, results) => {
    if (err) return res.status(500).json({ error: "Erro ao buscar chamados pendentes" });
    res.json(results);
  });
});

// Listar chamados do técnico logado
router.get("/meus-chamados/:tecnicoId", (req, res) => {
...
// Assumir chamado (atribuir ao técnico)
router.put("/assumir/:id", (req, res) => {
  const chamadoId = req.params.id;
  const { tecnico_id } = req.body;

  if (!tecnico_id) return res.status(400).json({ error: "tecnico_id é obrigatório" });

  const query = `UPDATE chamados SET tecnico_id = ?, status = 'em andamento' WHERE id = ?`;
  db.query(query, [tecnico_id, chamadoId], (err, result) => {
    if (err) return res.status(500).json({ error: "Erro ao assumir chamado" });
    if (result.affectedRows === 0) return res.status(404).json({ error: "Chamado não encontrado" });
